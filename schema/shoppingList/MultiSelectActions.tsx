...
export function MultiSelectActions(props: ArrayFieldProps) {
  const {inputProps} = props
  // const {value, onChange, readOnly} = inputProps
  // IWBN if the ArrayFieldProps took the value type and passed it to inputProps
  const {value, onChange, readOnly} = inputProps as ArrayOfObjectsInputProps<ListItemValue>
  const {selected, selectAll} = useMultiSelectContext()
  const _type = useFormValue(['_type'])

  const localStorageId = `multi-select-${props.inputId}-${_type}`
  ...
  return (
    <MenuButton
      button={<Button mode="bleed" icon={EllipsisVerticalIcon} disabled={readOnly} />}
